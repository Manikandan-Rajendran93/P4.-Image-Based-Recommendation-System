...
def stored_resources():
    ...
    resnet_model = torch.load("resnet50_feature_extractor.pt", weights_only = False)
...
tab_visual, tab_metric = st.tabs(["💻 VISUAL RECOMMENDATION WINDOW", "📈 EVALUATION METRIC WINDOW"])
...
with tab_metric:
    ...
    evaluation_metrics, = st.columns(1)
    with evaluation_metrics:
        ...
        kpi_columns[1].metric(
            label = "Unique Items Utilized by Engine out of 10000 Products ", 
